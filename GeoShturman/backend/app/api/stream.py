...
class StreamHub:
    ...
    async def _run_solve(self, window: list[dict[str, Any]]) -> None:
        try:
            ...
            # захват по всей области (центр поиска = центр карты = точка старта)
            solution = await loop.run_in_executor(None, _solve_window, window, self.baro_msl, None, hz)
            if solution:
...
def _solve_window(window: list[dict[str, Any]], baro_msl: float,
                  ...
                  sample_hz: float = 5.0,
                  speed_lo: float = 20.0, speed_hi: float = 80.0) -> dict[str, Any] | None:
    ...
    try:
        ...
        ex = float(est.get("end_x_m", est.get("start_x_m", 4000)))
        ey = float(est.get("end_y_m", est.get("start_y_m", 4000)))
        lat, lon = _meters_to_latlon(ex, ey)
        ...
        return {
            ...
            "start_x_m": round(float(est.get("start_x_m", ex)), 1),
            "start_y_m": round(float(est.get("start_y_m", ey)), 1),
